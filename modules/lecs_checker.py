...
def assess_text_originality(text):
    ...
    if diversity < 0.3:
        return f"{diversity:.2f}. Текст неоригінальний"
    elif 0.3 <= diversity <= 0.7:
        return f"{diversity:.2f}. Текст оригінальний"
    elif 0.7 < diversity < 1.0:
        return f"{diversity:.2f}. Текст високооригінальний"
    elif diversity == 1.0:
        return f"{diversity:.2f}. Текст підозріло штучний"
